...
function check_product()
{
    ...
    if (echo -n $1 | grep -q -e "^xylon_") ; then
       XYLON_BUILD=$(echo -n $1 | sed -e 's/^xylon_//g')
    else
       XYLON_BUILD=
    fi
    export XYLON_BUILD
...
function print_lunch_menu()
{
    ...
    echo
    if [ "z${XYLON_DEVICES_ONLY}" != "z" ]; then
...
function lunch()
{
    ...
    if [ $? -ne 0 ]
    then
        # if we can't find a product, try to grab it off the XYLON github
        T=$(gettop)
        pushd $T > /dev/null
...
{
    if [ "$OUT" ] ; then
        MODVERSION=`sed -n -e'/ro\.xylon\.version/s/.*=//p' $OUT/system/build.prop`
        ZIPFILE=xylon-$MODVERSION.zip
        ZIPPATH=$OUT/$ZIPFILE
        if [ ! -f $ZIPPATH ] ; then
            echo "Nothing to eat"
            return 1
    ...
    if (adb shell cat /system/build.prop | grep -q "ro.xylon.device=$XYLON_BUILD");
    then
...
EOF
    ...
    return $?
        ...
        echo "The connected device does not appear to be $XYLON_BUILD, run away!"
    fi
}
